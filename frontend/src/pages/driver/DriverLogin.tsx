...
import { Navigation, KeyRound, User, School, ArrowRight, AlertCircle } from 'lucide-react';
...
export const DriverLogin: React.FC = () => {
  ...
  useEffect(() => {
    authApi.getColleges()
      ...
      .catch((err) => {
        console.warn('Backend colleges fetch failed, using fallback list:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Invalid Driver credentials. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await authApi.driverRecoverPin({
      ...
      if (res.data.success) {
        setForgotMsg(res.data.message || 'Driver PIN updated successfully!');
      ...
      setForgotError(err.response?.data?.error || 'Failed to reset Driver PIN. Please check your Driver Name or Phone.');
    } finally {
    ...
    <div className="min-h-screen flex flex-col justify-between">
      ...
      <div className="flex-1 flex flex-col justify-center py-8 sm:px-6 lg:px-8 px-4">
        <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-3">
          ...
          <h2 className="text-3xl font-black tracking-tight drop-shadow-sm text-slate-900 dark:text-white card-title">Driver Login</h2>
          <p className="text-sm font-semibold subtext-muted text-slate-600 dark:text-slate-300">
            SRGEC Bus Driver portal - live route and GPS navigation.
          </p>
        </div>
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div className="water-glass py-8 px-6 sm:px-10 rounded-3xl shadow-2xl">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="e.g. 9876543210 or DRV-01"
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    className="text-xs text-emerald-500 hover:text-emerald-400 font-bold transition-colors underline"
                  >
                    Forgot PIN?
                  </button>
                </div>
            ...
            <div className="mt-6 text-center text-xs font-bold subtext-muted">
              New Driver?{' '}
              <Link to="/driver/register" className="link-emerald underline ml-1">
                Register here
              </Link>
            </div>
          </div>
          ...
          <div className="water-glass max-w-md w-full p-6 sm:p-8 rounded-3xl shadow-2xl border border-white/20 space-y-5 relative">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-black text-white">Reset Driver PIN</h3>
                  <span className="text-[11px] text-slate-400">Instant Driver PIN Recovery</span>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleForgotSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-black uppercase tracking-wider mb-1 form-label">
                  Your Driver Name or Phone Number
                </label>
                <input
                  type="text"
                  value={forgotIdentifier}
                  onChange={(e) => setForgotIdentifier(e.target.value)}
                  placeholder="e.g. Ramesh or 7036349676"
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-black text-xs shadow-lg shadow-emerald-500/25 transition-all disabled:opacity-50"
                >
