  ...
  return (
    <div className="min-h-screen text-slate-100 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* Main Grid: Live Satellite Map (Top on Mobile) & Telemetry Details */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Secondary Column: Uber-Style Status Card & Details */}
          <div className="order-2 lg:order-1 space-y-6">
            ...
            {selectedBus?.route?.boardingPoints && selectedBus.route.boardingPoints.length > 0 && (
              <div className="water-glass rounded-3xl p-5 sm:p-6 shadow-2xl space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-black uppercase tracking-wider text-slate-200">
                    Route Stops ({selectedBus.route.name})
                  </h4>
                  <button
                    onClick={() => setShowProfileModal(true)}
                ...
                <div className="space-y-3 relative pl-4 before:absolute before:left-1.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-white/20">
                  {selectedBus.route.boardingPoints
                    .sort((a, b) => a.sequence - b.sequence)
                    .map((stop) => {
                      const isStudentStop = stop.id === boardingPoint?.id;
                      return (
                        <div key={stop.id} className="relative flex items-center justify-between text-xs">
                          <div
                            className={`absolute -left-4 w-3.5 h-3.5 rounded-full border-2 ${
                              isStudentStop
                          ...
                          <span
                            className={`font-semibold ${
                              isStudentStop ? 'text-cyan-300 font-bold' : 'text-slate-200'
                            }`}
                          >
                            {stop.name} {isStudentStop && '(Your Boarding)'}
                          </span>
                          <span className="text-slate-400 font-mono">#{stop.sequence}</span>
                        </div>
                      );
                    })}
                </div>
